...
export interface CallConfig {
  ...
  isGroup?: boolean;
  callId?: string;
  authToken?: string; // RTK auth token
  meetingId?: string; // RTK meeting ID
}
...
async function getRtkToken(roomName: string, displayName: string, isVideoCall: boolean): Promise<{ authToken: string; meetingId: string }> {
  const { data: { session } } = await supabase.auth.getSession();
  if (!session?.access_token) throw new Error('Not authenticated');

  const { data, error } = await supabase.functions.invoke('generate-rtk-token', {
    headers: { Authorization: `Bearer ${session.access_token}` },
    body: { roomName, displayName, isVideoCall },
  });

  if (error || !data?.authToken) {
    console.error('RTK token error:', error, data);
    throw new Error('Token generation failed');
  }
  return { authToken: data.authToken, meetingId: data.meetingId };
}

export { getRtkToken };

export const useRealtimeKitCall = (currentUserId: string) => {
